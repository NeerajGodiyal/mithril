...
func (voteInit *VoteInstrVoteInitV2) UnmarshalWithDecoder(decoder *bin.Decoder) error {
	...
	blsPop, err := decoder.ReadBytes(blsProofOfPossessionCompressedSize)
	if err != nil {
		return err
	}
	copy(voteInit.AuthorizedVoterBLSProofOfPossession[:], blsPop)
...
func VoteProgramInitializeAccountV2(
	...
	inflationCollector, err := resolveVoteCommissionCollector(
		txCtx, instrCtx, voteKey, 2, rent,
	)
	if err != nil {
		return err
	}
	blockRevenueCollector, err := resolveVoteCommissionCollector(
		txCtx, instrCtx, voteKey, 3, rent,
	)
	if err != nil {
		return err
	}
...
func VoteProgramAuthorize(execCtx *ExecutionCtx, voteAcct *BorrowedAccount, authorized solana.PublicKey, authorize VoteAuthorizeKind, signers []solana.PublicKey, clock SysvarClock, f features.Features) error {
	...
	case VoteAuthorizeTypeVoter:
		if voteStateHasBLSPubkey(voteState) {
			return InstrErrInvalidInstructionData
		}
