...
func TestLtHash_1(t *testing.T) {
	input := []byte("hello")
	ltHash := new(LtHash).InitWithBytes(input)

	outputChecksum := ltHash.Checksum()
	expectedChecksum := []byte{79, 156, 26, 184, 156, 205, 94, 208, 182, 235, 33, 147, 111, 153, 229, 152, 207, 133,
		75, 109, 182, 198, 119, 61, 11, 81, 41, 70, 24, 87, 100, 85}
	assert.Equal(t, expectedChecksum, outputChecksum)

	outputHash := ltHash.Hash()
	expectedHash := []uint16{0x8fea, 0x3d16, 0x86b3, 0x9282, 0x445e, 0xc591, 0x8de5, 0xb34b, 0x6e50, 0xc1f8, 0xb74e,
	...
	expectedHashSlice := unsafe.Slice((*uint8)(unsafe.Pointer(&expectedHash[0])), numElements*2)
	assert.Equal(t, expectedHashSlice, outputHash)
}

func TestLtHash_2(t *testing.T) {
	input := []byte("world!")
	ltHash := new(LtHash).InitWithBytes(input)

	outputChecksum := ltHash.Checksum()
	expectedChecksum := []byte{171, 53, 185, 10, 179, 49, 48, 151, 87, 43, 141, 13, 43, 152, 121, 1, 144, 7, 120, 188,
		115, 248, 214, 220, 229, 210, 175, 134, 215, 231, 18, 245}
	assert.Equal(t, expectedChecksum, outputChecksum)

	outputHash := ltHash.Hash()
	expectedHash := []uint16{0x56dc, 0x1d98, 0x5420, 0x810d, 0x936f, 0x1011, 0xa2ff, 0x6681, 0x637e, 0x9f2c, 0x0024,
...
func TestLtHash_Inverse(t *testing.T) {
	a := new(LtHash).initRandom()
	b := new(LtHash).initRandom()

	copyA := a.Clone()
	result1 := copyA.Add(b).Sub(b)
	assert.Equal(t, *a, *result1)

	copyA = a.Clone()
	result2 := copyA.Sub(b).Add(b)
	assert.Equal(t, *a, *result2)
}

func TestLtHash_Commutative(t *testing.T) {
	a := new(LtHash).initRandom()
	b := new(LtHash).initRandom()

	result1 := a.Clone().Add(b)
	result2 := b.Clone().Add(a)
