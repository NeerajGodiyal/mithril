...
func TestLimitsForFeaturesRaiseBlockLimitsTo100m(t *testing.T) {
	...
	assert.Equal(t, uint64(40_000_000), LimitsForFeatures(feats).WritableAccountCost)
	assert.Equal(t, LimitsForSlotNanos(400_000_000, true), LimitsForFeatures(feats))
	assert.Equal(t, uint64(MaxBlockUnitsSIMD0256), DefaultLimits().BlockCost)
}

func TestRaisedCostLimitsScaleWithSlotDuration(t *testing.T) {
	for _, test := range []struct {
		nanos, blockCost, accountCost uint64
	}{
		{400_000_000, 100_000_000, 40_000_000},
		{350_000_000, 87_500_000, 35_000_000},
		{300_000_000, 75_000_000, 30_000_000},
		{250_000_000, 62_500_000, 25_000_000},
		{200_000_000, 50_000_000, 20_000_000},
	} {
		limits := LimitsForSlotNanos(test.nanos, true)
		assert.Equal(t, test.blockCost, limits.BlockCost, "slot duration %d", test.nanos)
		assert.Equal(t, test.accountCost, limits.WritableAccountCost, "slot duration %d", test.nanos)
	}
}
