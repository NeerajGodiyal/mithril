...
func TryEnqueue(ingress chan<- packet.Packet, pkt packet.Packet, stats *IngressStats) bool {
	...
	case ingress <- pkt:
		if stats != nil {
			stats.EnqueuedPackets++
			stats.EnqueuedBytes += uint64(pkt.Len())
		}
		return true
	default:
		if stats != nil {
...
func runSigverifyWorker(
	...
	for pkt := range in {
		...
		if !verifyPacket(data) {
			stats.DroppedSigverify++
			pkt.Release()
			continue
		}

		stats.VerifiedPackets++
		stats.VerifiedBytes += uint64(len(data))
		out <- pkt
	}
}
