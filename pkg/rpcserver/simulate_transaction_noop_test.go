...
func TestEarlyProcessingResultsStillCollectTokenBalances(t *testing.T) {
	...
	tokenData[tokenAccountStateOffset] = 1

	slotCtx := &sealevel.SlotCtx{Accounts: accounts.NewMemAccounts()}
	mint := makeTokenMint(splTokenProgramID, 6)
	mint.Key = mintKey
	require.NoError(t, slotCtx.SetAccount(mintKey, mint))
	...
	pre, post := simulationAccountSnapshotsForOutput(slotCtx, tx, output, slotCtx.GetAccountFromAccountsDb)
	require.Len(t, pre, len(tx.Message.AccountKeys))
	require.Len(t, post, len(tx.Message.AccountKeys))
	preTokens := tokenBalancesForTransaction(tx, pre, slotCtx.GetAccount)
	postTokens := tokenBalancesForTransaction(tx, post, slotCtx.GetAccount)
