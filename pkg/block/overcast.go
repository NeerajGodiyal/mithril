...
// native block representation. Unknown protobuf fields are rejected so that
// unsupported message variants cannot be silently decoded as a supported version.
...
func overcastTransactionToTransaction(overcastTx *overcast.VersionedTransaction) (*solana.Transaction, uint8, error) {
	...
	if len(overcastTx.ProtoReflect().GetUnknown()) != 0 {
		return nil, 0, fmt.Errorf("unsupported transaction fields")
	}
	...
	case *overcast.VersionedTransaction_MessageV1:
		if message == nil || message.MessageV1 == nil {
			return nil, 0, fmt.Errorf("V1 transaction has no message payload")
		}
		header = message.MessageV1.Header
		accountKeys = message.MessageV1.AccountKeys
		recentHash = message.MessageV1.LifetimeSpecifier
		instructions = message.MessageV1.Instructions
		if cfg := message.MessageV1.Config; cfg != nil {
			config = solana.TransactionConfig{
	...
	default:
		return nil, 0, fmt.Errorf("unsupported or missing transaction message")
	}
	...
	if header.NumReadonlySignedAccounts > math.MaxUint8 ||
		...
		return nil, 0, fmt.Errorf("transaction header exceeds u8 limits")
	}
	if len(recentHash) != overcastHashSize {
		return nil, 0, fmt.Errorf("transaction has invalid lifetime hash length %d", len(recentHash))
	}
	...
	if err := txverify.SanitizeTransaction(tx); err != nil {
		return nil, 0, fmt.Errorf("invalid transaction version %d: %w", version, err)
	}
...
func overcastInstrToInstr(instr *overcast.CompiledInstruction) (solana.CompiledInstruction, error) {
	...
	if instr.ProgramIdIndex > math.MaxUint8 {
		return solana.CompiledInstruction{}, fmt.Errorf("program ID index %d exceeds u8 limit", instr.ProgramIdIndex)
	}
	...
	for idx, account := range instr.Accounts {
		if account > math.MaxUint8 {
			return solana.CompiledInstruction{}, fmt.Errorf("account index %d exceeds u8 limit", account)
		}
