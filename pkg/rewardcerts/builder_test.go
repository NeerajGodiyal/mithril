...
func TestBuilderDropsFarFutureVote(t *testing.T) {
	root := uint64(100)
	builder := NewBuilder(BuilderConfig{MaxValidators: 1, RootSlot: func() uint64 { return root }})
	builder.AddVote(testSignedVote(t, alpenglow.NewSkipVote(root+rewardVoteFutureWindow+1), 0, testBLSKeys(t, 1)[0]))
	assert.Empty(t, builder.votes)
}
...
func assertAggregateMatchesVotes(t *testing.T, aggregate []byte, votes []alpenglow.VoteMessage) {
	...
	require.NoError(t, verifier.VerifyRewardCertificateForEpoch(set.Epoch, cert))
	_ = keys
}
