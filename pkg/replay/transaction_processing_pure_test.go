...
func TestFeePayerSourceFailureCannotBecomeRelaxedNoOp(t *testing.T) {
	slotCtx, cleanup := newCommitTestSlotCtx()
	defer cleanup()
	slotCtx.Features.EnableFeature(features.RelaxFeePayerConstraint, 0)
	delete(slotCtx.Accounts.(accounts.MemAccounts).Map, txfixture.PayerPubkey())
	slotCtx.UnrootedRead = failingAccountSource{}
	tx, err := solana.TransactionFromBytes(txfixture.MustSignedTransferWire(0))
	require.NoError(t, err)

	out := LoadAndExecuteTransaction(LoadAndExecuteTransactionInput{SlotCtx: slotCtx, Transaction: tx})
	require.ErrorIs(t, out.LoadError, errTestAccountSource)
	require.ErrorIs(t, out.LoadError, fees.ErrFeePayerSource)
	require.Nil(t, out.ProcessingResult.TransactionError)
	require.False(t, out.ProcessedAsNoOp)
	require.False(t, out.FeesOnly)
	require.Empty(t, out.PostAccountSnapshots)
	require.Empty(t, slotCtx.ModifiedAccts)
	require.ErrorIs(t, fees.PayerCanFund(slotCtx, tx), errTestAccountSource)
}
