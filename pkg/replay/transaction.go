...
func recordStakeDelegation(slot uint64, acct *accounts.Account) {
	...
	if isEmpty || isUninitialized {
		// Debug: log stake account removals to track what's being closed during replay
		if global.HasStakeCacheItem(acct.Key) {
			mlog.Log.Debugf("STAKE_CACHE_REMOVE: slot=%d pubkey=%s reason=%s lamports=%d data_len=%d",
				slot,
				acct.Key.String(),
				func() string {
					if isEmpty {
