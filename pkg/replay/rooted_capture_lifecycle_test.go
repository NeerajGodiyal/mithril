...
func TestRootedCaptureRetainsOwnedEvidenceUntilFoldOrUnwind(t *testing.T) {
	...
	observations := []rootedevents.TransactionObservation{{Message: []byte{1, 2}, Logs: []string{"original"}}}
	require.NoError(t, tail.RecordRootedEventSlot(20, 19, observations))
	retainedBytes := tail.transactionBytes
	observations[0].Message[0] = 9
	observations[0].Logs[0] = "changed"
	require.Equal(t, byte(1), tail.transactions[20][0].Message[0])
	require.Equal(t, "original", tail.transactions[20][0].Logs[0])

	require.Error(t, tail.recordRootedEventSlotWithLimit(21, 20, observations, retainedBytes))
	require.NotContains(t, tail.parentSlots, uint64(21))
	require.NotContains(t, tail.transactions, uint64(21))
	require.Equal(t, retainedBytes, tail.transactionBytes)

	require.NoError(t, tail.RecordRootedEventSlot(21, 20, nil))
