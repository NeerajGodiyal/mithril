...
func TestWallClockSlotUsesConfiguredProtocolDuration(t *testing.T) {
	SetWallClockSlotDuration(200 * time.Millisecond)
	defer SetWallClockSlotDuration(defaultSlotDuration)

	wallClock.seed(100)
	wallClock.at = time.Now().Add(-800 * time.Millisecond)
	assert.Equal(t, uint64(104), WallClockSlot())
}
